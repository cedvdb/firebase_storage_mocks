...
class MockReference extends Mock implements Reference {
  ...
  Future<FullMetadata> updateMetadata(SettableMetadata metadata) {
    ...
    final newSettable = _getNewSettableFromFullMetadata(
        _storage.storedMetadata[_path], metadata);
  ...
  FullMetadata _createFullMetadata(SettableMetadata? metadata) {
    ...
    final Map<String, dynamic> newMetadata =
        metadata?.asMap() ?? <String, dynamic>{};

    newMetadata['bucket'] = _storage.storedMetadata[_path]?.bucket ?? bucket;
    newMetadata['fullPath'] =
        _storage.storedMetadata[_path]?.fullPath ?? fullPath;
    newMetadata['metadataGeneration'] =
        _storage.storedMetadata[_path]?.metadataGeneration ??
            'metadataGeneration';
    newMetadata['md5Hash'] =
        _storage.storedMetadata[_path]?.md5Hash ?? 'md5Hash';
    newMetadata['metageneration'] =
        _storage.storedMetadata[_path]?.metadataGeneration ?? 'metageneration';
    newMetadata['name'] = _storage.storedMetadata[_path]?.name ?? name;
    newMetadata['size'] = _storage.storedMetadata[_path]?.size ??
        _storage.storedDataMap[_path]?.lengthInBytes ??
        _storage.storedFilesMap[_path]!.lengthSync();
    newMetadata['creationTimeMillis'] =
        _storage.storedMetadata[_path]?.timeCreated?.millisecondsSinceEpoch ??
            DateTime.now().millisecondsSinceEpoch;
    newMetadata['updatedTimeMillis'] = DateTime.now().millisecondsSinceEpoch;
    return FullMetadata(newMetadata);
  }

  SettableMetadata _getNewSettableFromFullMetadata(
      FullMetadata? fullMetadata, SettableMetadata settableMetadata) {
    return SettableMetadata(
        cacheControl:
            settableMetadata.cacheControl ?? fullMetadata?.cacheControl,
        contentDisposition: settableMetadata.contentDisposition ??
            fullMetadata?.contentDisposition,
        contentEncoding:
            settableMetadata.contentEncoding ?? fullMetadata?.contentEncoding,
        contentLanguage:
            settableMetadata.contentLanguage ?? fullMetadata?.contentLanguage,
        contentType: settableMetadata.contentType ?? fullMetadata?.contentType,
        customMetadata:
            settableMetadata.customMetadata ?? fullMetadata?.customMetadata);
  }
